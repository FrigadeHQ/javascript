...
export function Editor() {
  const [items, setItems] = useState({
    'sortable-card': ['sortable-title', 'sortable-subtitle'],
  })
  ...
  function handleDragEnd({ active, over }) {
    ...
    if (active.id !== over.id) {
      const dropId = over.data.current.sortable.containerId

      setItems((items) => {
        const oldIndex = items[dropId].indexOf(active.id) ?? 0
        const newIndex = items[dropId].indexOf(over.id)
  ...
  return (
    <>
      ...
      >
        ...
        <DragOverlay>{activeId && <DraggableTitle>Whew ok.</DraggableTitle>}</DragOverlay>
      </DndContext>
    </>
  )
}
